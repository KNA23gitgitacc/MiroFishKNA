...
class Config:
    ...
    # JSON config — display CJK characters directly (not \uXXXX)
    JSON_AS_ASCII = False

    # LLM config (unified OpenAI format — works with Ollama via OpenAI-compatible API)
    LLM_API_KEY = os.environ.get('LLM_API_KEY', 'ollama')
    LLM_BASE_URL = os.environ.get('LLM_BASE_URL', 'http://localhost:11434/v1')
    LLM_MODEL_NAME = os.environ.get('LLM_MODEL_NAME', 'qwen2.5:32b')

    # Neo4j config (replaces Zep Cloud)
